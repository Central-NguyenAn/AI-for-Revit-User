...
- Rebuild for `Release2023`, `Release2024` configurations.
...
Sau khi cập nhật code xong, AI MUST luôn luôn tự build lại dự án cho cả 2 cấu hình (Release2023 và Release2024). Nếu có lỗi build, AI phải tự phân tích lỗi, chỉnh sửa lại code và tiếp tục build cho đến khi hết lỗi thì mới báo cáo kết quả cho người dùng.
...
2. **GỌI API NGAY** theo workflow — KHÔNG CẦN scan project, KHÔNG CẦN đọc source code C#, KHÔNG CẦN list_dir hay grep_search.
...
AI **TUYỆT ĐỐI KHÔNG ĐƯỢC** tự viết thêm handler, endpoint, sửa code C#, hay tạo tính năng mới khi đang xử lý yêu cầu của user. Nếu yêu cầu không thể thực hiện bằng các endpoint/tool hiện có:
...
## Rule 9: Cấu trúc Source Code
```
AI for Revit/
├── RevitBridge/                   # C# Plugin (chạy trong Revit)
│   ├── Application.cs            # Entry point: HTTP Server + Idling event
│   ├── Core/
│   │   ├── HttpServer.cs         # HTTP server localhost:8080
│   │   ├── RequestRouter.cs      # Route HTTP request → Handler
│   │   └── RevitApiHandler.cs    # Thread-safe Revit API access
│   ├── Handlers/                 # Xử lý logic Revit API
│   ├── Models/                   # Request/Response models
│   ├── PackageContents.xml       # Bundle manifest
│   └── RevitBridge.sln
├── .agent/                       # AI tri thức + workflows
├── dist-template/                # Template cho bản phân phối
├── package.ps1                   # Script đóng gói
└── README.md
```
