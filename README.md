...
Cơ sở dữ liệu lệnh (`openapi.json`) được đính kèm sẵn dưới dạng tệp cấu hình `.agent`, cho phép AI nhận diện phương thức giao tiếp mà không cần cấu hình phức tạp.
...
Hệ thống cung cấp các phương thức thao tác hàng loạt qua HTTP API, bao gồm:
...
- **Batch Parameter Update:** Cho phép ghi/đè tham số cho hàng loạt Element ID chỉ với một lệnh (REST API payload).
- **MEP Analysis:** Khả năng truy vết mạng lưới (network tracing) của hệ thống Cơ Điện (MEP), kiểm tra số lượng Sprinkler hạ nguồn và đối soát vùng bao phủ an toàn.
- **LLM-Compact Data Formatter:** Kết xuất dữ liệu JSON ở cấu trúc cô đọng (loại bỏ Null và ký tự định dạng) để tương thích với giới hạn bộ nhớ (Context Window) của LLM.
...
**Xử lý tham số:**
> *"Tìm tất cả Pipes thuộc tần 'Tầng 1' có kích thước Diameter lớn hơn 100mm."*  
> *"Gán tham số 'Comments' thành 'Tested' cho toàn bộ Doors tại Tầng 2 đang được chọn."*
